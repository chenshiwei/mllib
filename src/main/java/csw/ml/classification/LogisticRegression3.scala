...
object LogisticRegression3 extends App {
  ...
  val training = spark.sparkContext.parallelize(Seq(
...
//    .setWeightCol("weight")
  ...
  val testData = spark.createDataFrame(Seq(
    ...
    (1.0, Vectors.dense(2.0, 2.2, -1.0)))).toDF("label", "features")
  model1.transform(testData).show(false)
